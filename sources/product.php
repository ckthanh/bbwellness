<?php  if(!defined('_source')) die("Error");
	...
	else if($id!='')
	{
		...
		//Chi tiết sản phẩm
		$d->reset();
		$sql_detail = "select id,ten$lang as ten,mota$lang as mota,noidung$lang as noidung,masp,gia,giacu,luotxem,thumb,photo,size,mausac,id_danhmuc,id_list,id_cat,title,keywords,description FROM #_product where  tenkhongdau='$id' and hienthi=1 limit 0,1";
		$d->query($sql_detail);
	...
	else
	{
		...
		if($com=='san-pham-khuyen-mai')
		{
			$where = " type='".$type."' and spmoi>0 and hienthi=1 order by stt,id desc";
			$com= "san-pham";
